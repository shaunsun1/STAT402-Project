...
\headerbox{Assumptions}{name=Assumptions,column=0,below=Imputation}{
}
...
\headerbox(Model Theory}{name=Methods,column=1,span=1,below=Imputation}{
...
\item Following Lumley..., the score function above was altered to incorporate design weights, and this was solved to get the parameter estimates:
...
\headerbox{Exploratory Analysis}{name=EDA,column=0,below=Methods}{

\textbf{Purpose}: To determine whether higher-order terms should be included in the models.
\item In these plots, the continuous variables were made into categorical variables to avoid observed hypertension proportions of 0 or 1.
...
\headerbox{Checking Model Fit}{name=Model Fit,column=0,below=EDA}{
}

\headerbox{Hypothesis Testing Theory}{name=Testing,column=0,below=Model Fit}{
...
\item Under the null hypothesis, this statistic follows a Chi-Square distribution with q degree of freedom.
...
\item Following Lumley..., the standard likelihood function was altered to include weights:
...
Finally, $/tau$ has a rather complex distribution under $H_{0}$, but the saddlepoint approximation (Kuonen, 1999) can be used to approximate it.
}

\headerbox{Results}{name=Results,column=0,below=Testing}{
}
