...
\textbf{Objective}: Estimate the yearly \textcolor{red}{exploitation rates} of cod (Gadus Morhua) in coastal Newfoundland \\

\textbf{Study}: Fisheries and Ocean Canada conducted a \textcolor{red}{capture-recapture} study from 1997-2011.
\begin{itemize}
\item Approximatively 124\thousandseparator 000 fish were catched, tagged and released in different regions.
\item Fishers were offered rewards for returning the tags ($\approx$ 21\thousandseparator 000 fish reported).
\item Fish were tagged either with a single low-reward (10\$) front tag (78 \%), a single high-reward (100\$) front tag (13 \%) or two low-reward front and back tags (9 \%).
\end{itemize}

\textbf{Data}: The following information was recorded for both release and recapture (provided the tag was returned): length, NAFO area, date (week near) and tag condition, that is:
\begin{itemize}
\item S: Front tag on single low-reward tagged fish  %without tag lost
\item DF: Front tag only on double tagged fish %that lost its back tag %with front tag only %with back tag lost
\item DB: Back tag only on double tagged fish% that lost its front tag %with back tag only %with front tag lost
\item DD: Front and back tags on double tagged fish %(no tag loss)
\item H: Front tag on single high-reward tagged fish. %without lost.
\end{itemize}
%
...
\headerbox{Descriptive Statistics}{name=Descriptive,column=0,below=Introduction}{

\begin{multicols}{2}
\textbf{Time unit and region}:

\vspace{0.1cm}

We use yearly data and merge areas in 4 regions as follows:
\includegraphics[width=10em]{map4.png}
...
Migration between region from release (left) to catch (top).
...
\begin{multicols}{2}
\textbf{Releases and reports}:

\vspace{0.1cm}

blablab lablablabl ablabl ablablablabla blablabla \\
\includegraphics[width=14em]{plot_rec.png}
\end{multicols}

\textbf{Tag Retention}: Double tag releases were recovered 72\% as DD, 19\% as DB and 9\% as DF.\\
}
